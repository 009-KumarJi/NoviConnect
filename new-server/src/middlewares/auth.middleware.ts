...
const isAuthenticated = TryCatch((req, res, next) => {
  ...
  const decodedData = jwt.verify(token, process.env.JWT_SECRET as string) as jwt.JwtPayload;
...
const socketAuthenticator = TryCatch(async (err, socket, next) => {
  ...
  const decodedData = jwt.verify(authToken, process.env.JWT_SECRET as string) as jwt.JwtPayload;
