...
io.on("connection", async (socket) => {
  ...
  // Join the personal room for easy cross-container targeting
  socket.join(userIdStr);

  await redis.sadd("online_users", userIdStr);
  const currentOnlineUsers = await redis.smembers("online_users");
  socket.broadcast.emit(ONLINE_USERS, currentOnlineUsers);
  ...
  socket.on(CHAT_JOINED, async ({userId, members}) => {
    ...
    io.to(getSockets(members)).emit(ONLINE_USERS, online);
  });
  ...
  socket.on(CHAT_LEFT, async ({userId, members}) => {
    const online = await redis.smembers("online_users");
    io.to(getSockets(members)).emit(ONLINE_USERS, online);
  });
  ...
  socket.on("disconnect", async () => {
    ...
    const remainingOnline = await redis.smembers("online_users");
    socket.broadcast.emit(ONLINE_USERS, remainingOnline);
